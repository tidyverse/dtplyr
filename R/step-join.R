step_join <- function(x, y, on, style, suffix = c(".x", ".y")) {
  ...
  if (style %in% c("semi", "anti")) {
    ...
    vars <- join_colorder(x$vars, y$vars, on = on, suffix = suffix)
  }
  ...
  if (style == "left") {
    ...
    vars_out_dt2 <- join_vars_dt_dplyr_left(x$vars, y$vars, on$x, on$y, suffix)
  } else if (style %in% c("right", "inner")) {
    vars_out_dt <- join_vars_dt(x$vars, y$vars, on$y)
    vars_out_dt2 <- join_vars_dt_dplyr_right(x$vars, y$vars, on$y, suffix)
  } else if (style == "full") {
...
join_colorder <- function(x_names, y_names, on, suffix) {
  vars <- dplyr:::join_cols(x_names, y_names, by = purrr::set_names(on$y, on$x), suffix = suffix)
  c(names(vars$x$out), names(vars$y$out))

  by <- dplyr:::standardise_join_by(purrr::set_names(on$y, on$x), x_names = x_names, y_names = y_names)
  suffix <- list(x = suffix[[1]], y = suffix[[2]])

  x <- x_names
  y_aux <- setdiff(y_names, c(by$x, by$y))
  x_is_aux <- !x_names %in% by$x
  x[x_is_aux] <- add_suffixes(x_names[x_is_aux], c(by$x, y_aux), suffix$x)

  y <- add_suffixes(y_names, x_names, suffix$y)
  y <- y[!y_names %in% by$y]
...
#' @noRd
#' column names as generated by `dplyr::left_join(x, y)` but in the order of
#' the data.table left join `y[x, on = on]`
join_vars_dt_dplyr_left <- function(x, y, on_x, on_y, suffix = c(".x", ".y")) {
  # rename y
  y_out <- purrr::set_names(y)
  idx <- vctrs::vec_match(on_y, y_out)
  y_out[on_y] <- on_x[idx]

  x_out <- setdiff(x, on_x)
  xy_out <- add_suffix(x_out, y_out, suffix)

  c(unname(xy_out$y), xy_out$x)
}

join_vars_dt_dplyr_right <- function(x, y, on_y, suffix = c(".x", ".y")) {
  y_out <- setdiff(y, on_y)
