...
context("arrange (tests from dplyr)")
...
test_that("arrange respects attributes #1105", {
  ...
  df <- data.table(p = Period(c(1, 2, 3)), x = 1:3)
  res <- arrange(df, p)
...
test_that("can choose to inclue grouping vars", {
  df <- data.table(g = c(1, 2), x = c(2, 1)) %>% group_by(g)
