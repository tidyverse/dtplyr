...
test_that("filter gives useful error message when given incorrect input", {
  ...
  expect_error(filter(tbl_dt(mtcars), `_x`),
    "_x",
    fixed = TRUE
  )
})
...
test_that("filter propagates attributes", {
  ...
  test <- data.table(Date = ISOdate(2010, 01, 01, 1:10))
...
test_that("filter handles $ correctly (#278)", {
  d1 <- tbl_dt(data.table(
    num1 = as.character(sample(1:10, 1000, T)),
    var1 = runif(1000),
    stringsAsFactors = FALSE))
  d2 <- data.table(num1 = as.character(1:3), stringsAsFactors = FALSE)
...
test_that("$ does not end call traversing. #502", {
  ...
  d <- expand.grid(Subject = 1:3, TrialNo = 1:2, Time = 1:3) %>% tbl_dt %>%
...
test_that("filter handles S4 objects (#1366)", {
  ...
  df <- data.table(x = Numbers(1:10, foo = 10))
  res <- filter(df, x > 3)
...
test_that("`vars` attribute is not added if empty (#2772)", {
  expect_identical(data.table(x = 1:2) %>% filter(x == 1), data.table(x = 1L))
})
