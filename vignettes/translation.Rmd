...
This vignette shows the details of how dtplyr translates dplyr expressions into data.table code. If you see places where you think I could generate better data.table code, please let me know!
...
*   Lazily. In this form, trigged by using `lazy_dt()`, no computation is 
    ...
    dplyr never modifies its inputs in place, so the lazy approach also ensures
    that dtplyr never makes more than single copy.
...
Note that dplyr pledges to never modify the input data, so note that dtplyr automatically `copy()`s the input data.table.
...
Just like in dplyr, `group_by()` doesn't do anything by itself, but instead modifies the operation of downstream verbs. This general just involves using the `by` argument:
...
Over time, as I learn more about data.table, I hope to expand the set of these simplification.
