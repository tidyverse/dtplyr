...
Similarly, a `filter()` and `mutate()` can't be combined because `dt[a == 1, .(b2 := b * 2)]` would modifies the selected rows in place.
...
This is the default in order to match the existing dplyr interface. However, you can choose to opt out of this copy, and take advantage of data.tables reference semantics (for more details see `vignette("datatable-reference-semantics")`. Do this by setting `immutable = FALSE` on construction:

```{r}
dt_inplace <- lazy_dt(df, immutable = FALSE)
dt_inplace %>% mutate(a2 = a * 2, b2 = b * 2) %>% show_query()
```

Note that dtplyr does it's best to avoid needless copies, so it won't explicitly copy if there's already an implicit copy produced by `[`, `head()` or similar:
