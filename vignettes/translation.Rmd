...
This is the default in order to match the existing dplyr interface. However, you can choose to opt out of this copy, and take advantage of data.tables reference semantics (for more details see `vignette("datatable-reference-semantics")`. Do this by setting `immutable = FALSE` on construction. This only makes sense when the input is already a data table:
